...
describe("compile — response.onError (#81, ADD-12 §8.1)", () => {
  it("lowers onError, canonicalizing errorResource the same way resource is", () => {
    ...
    expect(tool.response?.onError).toEqual({
      errorResource: "shop.RowError",
      when: { path: "$.code", exists: true },
    });
    rmSync(dir, { recursive: true, force: true });
  });
});
